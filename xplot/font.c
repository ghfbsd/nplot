...
*/
{
   ...
   /* Not already loaded.  Read in from the font file. */
   {
      ...
      fontfile = NULL;
      sprintf(filename,"%s%03d\0",file_prefix,code);
   ...
   id of the prevailing font at the beginning of the string. */
{
   ...
   while (*sp != fc_end_of_string) {
	 ...
	 case fc_command_sr: /* Relative font size change */
	    rsize += (int)sp[1]; 
	    scale = (rsize + pfd->fd_height)/((float)(pfd->fd_height));
	    sp++;
	    break;
	 ...
	 default: /* May be either a character or some undefined code. */
	    if ( ((int) c) > MAX_CODE) {
	       char error_msg[128];
	       error(error_msg, 
		  "Internal error - unrecognized FC %d, giving up.",(int)c);
	    }
   ...
   given by code.  */
{
   ...
   /* Translate string. */
   if (strlen(characters) > max_length-1) {
      char ccopy[sizeof(fs)/sizeof(enum fontcode)];

      fprintf(stderr,"%s: Warning -- string too long, truncated.\n",
	 progname);
      strncpy(characters, ccopy, (int) sizeof(ccopy));
      translate(ccopy, fs, code);
   } else
      translate(characters, fs, code);
