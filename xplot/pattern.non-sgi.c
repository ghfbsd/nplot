...
dopat()
{
    ...
    if (patinit || (patused[patnum] == 0)) {
	  ...
	  image = XCreateImage(dpy,info->visual,depth,type,
	     0,NULL,8,8,8,0);
	  XFree(info);
	  if (depth>1) 
	     image->data = (char *)malloc(8*8*sizeof(unsigned long));
	  patinit = 0;
       }
       /* Construct bit map for image, copy to all planes. */
       if (depth == 1) {
	  image->data = (char*)&pats[patnum*2];
       } else {
	  XGCValues gcv;
	  ...
	  for(i=0;i<8*8;i++) {
       ...
       XPutImage(dpy,new,gc,image,0,0,0,0,8,8);
       patused[patnum] = new;
    }
